...
実験的なプロファイルは現在 macOS と固定 Codex バージョンが対象です。
...
最新のオフライン検証は 1,403 ケース通過しました。以前の自動化ベースラインではモデルを呼ばないサンドボックス検査 9 件も通過しています。
最後の実サブスクリプション検査では、Luna が有効な承認を期限切れと誤判断し、Sol の呼び出し前に停止しました。
自動実行は実験段階であり、信頼性や利用枠の節約が実証された機能ではありません。
