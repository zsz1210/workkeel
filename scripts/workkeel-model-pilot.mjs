...
export const cases = [
  ...
  { id: "ready", prompt: "Implement export readyTasks(tasks, state, limit) in src/solution.mjs. Valid inputs only: tasks have unique string id and dependencies arrays; state has completed/running/failed ID arrays; limit is a nonnegative integer. Return IDs in original task order whose dependencies are ALL in completed, excluding IDs in completed, running or failed. Missing dependency IDs and cycles simply remain not ready. Dependencies are not implicitly completed by selecting a task in this call. Limit caps returned count. Do not mutate anything. No dependencies or I/O.",
    ...
    checks: [
      ...
      ["all dependencies", "readyTasks([{id:'c',dependencies:['a','b']},{id:'d',dependencies:['a']}],{completed:['a'],running:[],failed:[]},9)", ["d"]],
      ...
      ["stable", "readyTasks([{id:'z',dependencies:['done']},{id:'a',dependencies:['done']}],{completed:['done'],running:[],failed:[]},9)", ["z","a"]],
